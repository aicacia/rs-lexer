...
pub struct Readers<T, E>(Vec<Arc<Reader<T, E>>>);
    ...
    E: 'static,
{
    ...
    pub fn remove<R: 'static + Reader<T, E>>(&mut self) {
        ...
        if let Some(index) = self.0.iter().position(|r| (&**r).get_type_id() == type_id) {
            self.0.remove(index);
        }
    }
}
...
impl<T, E> Readers<T, E> {
    ...
    #[inline]
    pub fn get(&self, index: usize) -> Option<&Arc<Reader<T, E>>> {
        self.0.get(index)
    }
...
where
    ...
    E: 'a,
{
    iter: slice::Iter<'a, Arc<Reader<T, E>>>,
}
